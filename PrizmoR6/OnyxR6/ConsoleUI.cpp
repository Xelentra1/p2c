...
void UI::Setup() {
	...
	consoleInfo.ColorTable[1] = 0xff0095; //0xffdd00 //0x10109c // a11f1f
...
void UI::ConsoleThread() {
	...
	int sideSize = (cwidth - 12) / 2;
	CPrint(std::wstring{ L"┏" } + MultStr(std::wstring{ L"━" }, sideSize) + std::wstring{ L"<PrizmoR6>" } +MultStr(std::wstring{ L"━" }, sideSize) + std::wstring{ L"┓" });
	...
	*/
	...
	const char* text = "By: Cyrex & int3nse";
